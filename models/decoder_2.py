...
class DynamicConvFacesObjectsDecoder(Decoder):
    def __init__(self, config, vocab, namespace="target_tokens"):
        super().__init__()
        vocab_size = config.vocab_size or vocab.get_vocab_size(namespace)
        ...
        input_embed_dim = self.embedder.get_output_dim()
        embed_dim = input_embed_dim
        output_embed_dim = input_embed_dim

        self.project_in_dim = GehringLinear(input_embed_dim, embed_dim, bias=False) if embed_dim != input_embed_dim else None
        ...
        if config.adaptive_softmax_cutoff is not None:
            adaptive_inputs = self.embedder if isinstance(self.embedder, AdaptiveEmbedding) else None
            if config.tie_adaptive_weights and adaptive_inputs is None:
            ...
            self.embed_out = nn.Parameter(torch.Tensor(vocab_size, output_embed_dim))
            nn.init.normal_(self.embed_out, mean=0, std=output_embed_dim ** -0.5)

        self.project_out_dim = GehringLinear(embed_dim, output_embed_dim, bias=False) if embed_dim != output_embed_dim and not config.tie_adaptive_weights else None

        self.normalize = config.decoder_normalize_before and config.final_norm
        self.layer_norm = nn.LayerNorm(embed_dim) if self.normalize else None
    ...
    def forward(self, prev_target, contexts, incremental_state=None, use_layers=None):
        ...
        if self.adaptive_softmax is None:
            if self.share_input_output_embed:
                X = F.linear(X, self.embedder.embed.weight)
            else:
    ...
    def get_normalized_probs(self, net_output, log_probs=False, sample=None):
        if self.adaptive_softmax is not None:
            ...
            return out if log_probs else out.exp()
